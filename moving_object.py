...
area=400                      # this area indicates that moving object detection area should be more than this 

while True:
    _,img=cam.read()         #read the image from camera
    text="Normal"            #Normal text when no object is detected
    img=imutils.resize(img,width=1000)                   # capturing window width =1000
    gray_img=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)        #convert the obtained image background to gray
    ...
    thresImg=cv2.dilate(threshImg,None,iterations=2)                  #dialte the image for 
    ...
    print(text)
    cv2.putText(img,text,(10,20),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,0,255),2)      #adding text to frame at (10,20) place co ordinates with red colour
